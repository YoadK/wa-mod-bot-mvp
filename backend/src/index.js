...
app.get('/api/whitelist', async (_req, res) => {
    const items = await Whitelist.find().sort({ createdAt: -1 }).lean();
    res.json(items);
});
...
app.post('/api/whitelist', async (req, res) => {
    ...
    if (!phone) return res.status(400).json({ error: 'phone is required' });
    await Whitelist.updateOne({ phone }, { $set: { note: note || 'manual' } }, { upsert: true });
    res.json({ ok: true });
});

app.delete('/api/whitelist/:phone', async (req, res) => {
    await Whitelist.deleteOne({ phone: req.params.phone });
    res.json({ ok: true });
});
